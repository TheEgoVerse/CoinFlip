...
export const coinFlip = async (event, setAlert, setDisableBtn, account) => {
    ...
    try {
        ...
        let contract = new web3.eth.Contract(contractABI, contractAddress, { from: fromAddress, gas: 1400000 })
        ...
        const contractResponse = await contract.methods.Flip(amount, web3.utils.toHex((rateOfWin * 10).toString())).send({ from: window.ethereum.selectedAddress, value: web3.utils.toWei('0', 'ether') })
        checkContract(contractResponse)
        function checkContract(contractResponse) {
            if (contractResponse.events && contractResponse.events.WinEvent) {
                const event = contractResponse.events.WinEvent
                ...
                let flipChoice = ''
                let winLose = ''
                if (event.returnValues.winner === false) {
                ...
                setTimeout(() => {
                    ...
                    if (!winLose) {
                        ...
                        return showAlert(setAlert, `Congratulation! You won ${event.returnValues.amount / 1000000000000000000} PREY!`, 'success')
                    }
                }, 3500);
